...
- Provide for pools of dynamically allocated/freed fixed or variable-length resources, allocated from the heap as needed, tracked and managed, then given back to the heap when done.  (Note that such pools have the overheads of both malloc and rsrc, but those overheads are often justified by the benefits in tracking and managing resources.)
...
- **resource handle = pxRsrcAlloc (pool handle to non-Var Pool, tracking name string)**, allocates a resource from any Pool except a Variable pool (since the length of a resource must be supplied in that case)
- **resource handle = pxRsrcVarAlloc (pool handle to Pool, tracking name string, resource size)**, allocates a resource of the specified size from the Pool. If a size is specified for a non-Var Pool, it must match the resource size of that Pool.

#### Miscellanous:

- **vRsrcRenameRsrc (resource handle, new tracking name string to associate with the resource)**
...
**vRsrcPrintLong (bufferpool);** // prints pool info, and tracking name and other info on each active resource

**vRsrcFree (buf);**  // frees any resource, whether variable, dynamic, static, etc., per pool behavior
...
The Free helper routine is called before adding a freed resource to the freelist.  It can unlink the resource from lists, free pointed-to memory, etc.  it is EXTREMELY important for this code to be cautious about null or stale pointers, since a resource can theoreticallly be in any state when it is freed.  (Note that Free helpers must have their own way of operating with variable-length resources, as the pool does not store each resource's length.  So, for example, the built-in clear-to-zero and clear-to-ones functions are no-ops for such resources since they cannot determine the resource length).

The Print helper routine is called when the built-in print routines are asked to print a resource.  It need not print, but is normally used to provide a custom printout of the struct, which is a huge time-saver when debugging vs. poking with the debugger or decoding hex.  As in the case for Free, it is important to be cautious of things like stale pointers (and especially NULL pointers), as the resource may be in virtually any state when printed.  The Pool data structure is itself a resource, and a default print helper is provided that prints the resource usage in a default format -- it can be replaced if desired by a customized one.  A simple PrintInHex routine is provided and can serve as a helper before, or instead of, writing a custom one for a resource type.
