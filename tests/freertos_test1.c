...
//	rsrc_test1
//	
//	Target test for rsrc package to run on FreeRTOS, derived from tests/test1.c
//	Build into a FreeRTOS program that includes the rsrc package.
//	Call from anywhere after application code is started.
//	Results will be printed on the log.
//
...
static const char *TAG = "rsrc_test1";
...
{
	ESP_LOGE (TAG, "User-suppied out-of-memory function was called.  Returning to continue...");
...
//	ESP_LOGI (TAG, "  Compiled with rsrcTEST_FORCE_OOM defined, so we'll try to run out of memory:");
	vRsrcOOMfn = OOMstub;
	...
	for (int i = 0; i < rsrcTEST_FORCE_OOM + 1; i++) {
		...
		if (!ignored) {
//			ESP_LOGI (TAG, "  Got a NULL back after %d allocations", i);
			goto success;
		}
	}
	return (TEST_FAILED);
...
int test_doublefree (void *res)
{
	...
	ESP_LOGE (TAG, "Attempting double-free on res.  If this results in abort(), the test PASSED.");
	vRsrcFree(res);
	vRsrcFree(res);
	ESP_LOGE (TAG, "DOUBLE-FREE TEST FAILED. This should not be printed, we should have aborted before now");
	return TEST_FAILED;
#else
	return (TEST_NOT_RUN);
...
{
	if (result < 0) {
		ESP_LOGI (TAG, "Test %s PASSED", testname);
	}
	else {
		if (result == 0)
			ESP_LOGW (TAG, "Test %s was NOT RUN", testname);
		else
			ESP_LOGE (TAG, "Test %s FAILED (%d)", testname, result);
	}
}

void rsrc_test1(void)
{
	...
	ESP_LOGI(TAG, "print short test:");
	vRsrcPrintShort(NULL);
	ESP_LOGI(TAG, "print long test:");
	pxRsrcVarAlloc (p3, "p3r1", 100); pxRsrcVarAlloc (p3, "p3r2", 200);
	pxRsrcAlloc (p1, "p1r1"); pxRsrcAlloc(p1, "p1r2");
	vRsrcPrintLong(NULL);
	...
	ESP_LOGI (TAG, "----> rsrc test finished normally");
}
